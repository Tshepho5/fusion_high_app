...
async function testOtp() {
  const reqGen = { body: { email: 'mapula@gmail.com' }, get: () => 'http://localhost:4000' };
  let genRes = null;
  ...
  const user = await db.query('SELECT reset_code, reset_expiry, NOW() as cur_time FROM users WHERE email = $1', ['mapula@gmail.com']);
  console.log('2. DB Record Expiry & Code:', user.rows[0]);
  
  const reqVer = { body: { email: 'mapula@gmail.com', otp: user.rows[0].reset_code } };
  let verRes = null;
  ...
  await db.query("UPDATE users SET reset_expiry = NOW() - INTERVAL '10 seconds' WHERE email = $1", ['mapula@gmail.com']);
  let expRes = null;
